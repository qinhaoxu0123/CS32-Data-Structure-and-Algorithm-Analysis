...
//  Homework 4
//
//  Created by Qinhao Xu on 3/6/17.
...
int main()
{
    ...
    // destruction time.
    {
        ...
        vector<Student*> pointerToStudenVector;
        for(int i= 0; i <auxStudents.size(); i++)
        {
            pointerToStudenVector.push_back(&auxStudents[i]);
        }
        ...
        sort(pointerToStudenVector.begin(), pointerToStudenVector.end(), compareStudentPtr);
        ...
        for(int j=0; j<pointerToStudenVector.size();j++)
        {
            students[j]=*pointerToStudenVector[j];
        }
